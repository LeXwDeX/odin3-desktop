...
2026-09-08，[v0.1.10](https://github.com/LeXwDeX/odin3-desktop/releases/tag/v0.1.10) 的[发布工作流](https://github.com/LeXwDeX/odin3-desktop/actions/runs/34190458943)和[对应主分支 CI](https://github.com/LeXwDeX/odin3-desktop/actions/runs/34190182328)均成功，标签与构建提交为 `019ef11412c711825d2c06d5975a4d89754372b7`。

原生界面的背景、卡片、文字和青色焦点改为更柔和的配色，常驻边框使用中性色，选中项通过边框与深色填充区分。性能与充电状态保留绿色、橙色、红色等信息颜色；存储分类的条形图、图例和数值共享相同颜色，内部存储和 TF 卡的空闲部分使用同一灰色。灯光菜单的色块仍代表真实硬件预设颜色。本次没有修改硬件控制、Dashboard 刷新周期或导航行为。

本地 Debug/Release 构建、42 项单元测试、279 个多语言键及架构回归通过，Lint 0 个错误、82 个警告。先安装同版本 Debug 构建，实测 Dashboard、全屏应用页、应用操作菜单和设置页的颜色及文字对比度；操作菜单只打开和关闭，没有执行删除或移动动作。

正式附件 SHA-256 为 `7485b91bcf7bc29323b2326cf5610094b0146ae74298c3f49fa49b7e44e248bf`，与 GitHub 资产摘要、校验附件及安装路径中的 APK 一致。包名 `com.odin.desktop`，版本 `0.1.10 / 1011`，不可调试，签名与已保留的 v0.1.9 一致。已在重新确认连接的 Odin3 / Android 15 上以 `adb install -r` 保留数据安装 GitHub 正式附件，并独立核对最终版本、哈希和应用数据目录 inode。
