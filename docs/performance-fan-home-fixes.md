...
短促 CPU 热点的 75°C 即时分支改为连续 4 秒确认，90°C 仍即时响应；温升／恢复期间每 2 秒采样，空闲保持 8 秒，且先采温再查询原厂硬件状态，避免短进程查询抬高采样热点。握持横屏允许竖屏应用，盖世游戏滤镜入口支持已有使用情况权限回退。实现、设备验证及边界见 [补充记录](grip-fan-shader-fixes.md)。
