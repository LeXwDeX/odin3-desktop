...
首版目标是简体中文和英文；皮肤、更多 Shader 和第三方适配先留可用接口。保留单个 `:app` Gradle 模块，以包和实际调用接口隔离职责，暂不引入插件 APK、动态代码加载或资源包安装器。
...
| UI、通知、磁贴和错误提示直接写中文，只有极少数 XML 字符串 | 英文作为完整默认资源，简体中文位于 `values-b+zh+Hans`；系统语言切换后重新创建的界面读取对应资源，通知频道与运行中服务同步刷新文案；注册系统应用语言列表 |
...
| 方向 | 首版落点 | 后续实现的位置 |
| --- | --- | --- |
| 多语言 | 完整 Android 字符串资源，语言无关的分类类型，用户名称原样保留 | 新增对应 `values-*` 资源并更新 `locales_config.xml`；应用内语言选择可以独立加入设置页 |
...
架构回归执行实际迁移 SQL，并与 Room 导出的 schema 比较；它还编译实际 Kotlin 分类和预设代码，验证语言切换不改变分类身份、改名不改变全集保护、需帧输入的效果分类和完整预设匹配。资源检查核对两种语言的 key 与格式参数。该 JVM 验证使用 Android 类型替身，未验证 Android 渲染、Room 运行时迁移回调和 JSON codec。

本机验证结果：Debug 和 Release 构建通过；Lint 为 0 个错误、60 个警告。警告主要涉及依赖版本、旧版本兼容判断、专用掌机固定横屏以及未使用资源等。`dispatchKeyEvent` 的 AndroidX Core 1.13.1 基类限制，以及 API 29–33 的磁贴 Intent 重载，均已核对依赖与版本分支后作局部说明；没有建立整库 Lint baseline。两种语言各 333 个字符串 key 校验通过；迁移、DAO 保护、预设与输入要求、91 项风扇协调器检查、高频 Home/Back、风扇完成通知和 9 组硬件 UI 回归通过，其中新增了灯光颜色/开关两种输入顺序的保留测试。

设备重连后需验证：中英文桌面/设置/OSD 的实际布局、长应用名及用户自定义分类名、切语言后的通知和磁贴；旧安装覆盖升级后的分类和排序；已有 Home/Back 与风扇自动策略待验项。使用相同 sRGB 基准图复验 GPU 截图效果与原画对比。本轮没有连接设备，因此本地构建和回归不等于完成这些验收。
