...
// GPL-3.0. FSR: Copyright (c) 2021 Advanced Micro Devices, Inc. MIT license.
...
vec3 applyFSR(vec2 uv, float sharp) {
    // EASU needs the actual source texture size; screen effects below use physical output pixels.
    vec2 inRes  = max(inputResolution, vec2(1.0));
